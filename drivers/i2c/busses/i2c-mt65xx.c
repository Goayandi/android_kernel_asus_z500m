...
struct mtk_i2c {
	...
	unsigned int clk_src_div;	/* i2c src clk div */
 ...
 */
static int mtk_i2c_calculate_speed(struct mtk_i2c *i2c, unsigned int parent_clk,
...
int mtk_i2c_set_speed(struct i2c_adapter *adap, u32 speed)
{
	int ret;
	struct clk *clk;
	...
	ret = mtk_i2c_calculate_speed(i2c, clk_get_rate(clk), i2c->clk_src_div);
	if (ret) {
		dev_err(i2c->dev, "Failed to calculate the i2c bus speed.\n");
		return -EINVAL;
	}
...
EXPORT_SYMBOL(mtk_i2c_set_speed);
...
static int mtk_i2c_probe(struct platform_device *pdev)
{
	...
	ret = mtk_i2c_parse_dt(pdev->dev.of_node, i2c, &i2c->clk_src_div);
	if (ret)
		return -EINVAL;

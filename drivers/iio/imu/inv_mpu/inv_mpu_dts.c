...
int inv_parse_pm(struct device *dev, struct mpu_platform_data *pdata);
...
int inv_parse_secondary(struct device *dev, struct mpu_platform_data *pdata)
{
	...
	if (of_property_read_string(np, "inven,secondary_type", &name)) {
		dev_info(dev, "Missing secondary type, leave it NULL\n");
		return 0;
	}
...
int inv_parse_pm(struct device *dev, struct mpu_platform_data *pdata)
{
	...
	rc = of_property_read_u32(np, "power_ctrl", &pm);
	if(IS_ERR(rc)) {
		dev_info(dev, "no power control, leave power variables NULL\n");
		return 0;
	}

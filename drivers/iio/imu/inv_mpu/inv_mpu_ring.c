...
//printk("t0=%lld\n", ts);
    ...
    st->int_cnt++;
	if (st->suspend_state == true) {
		st->wait_for_resume = 1;
	}
...
#if 1
#if 0
    thresh = st->irq_dur_ns >> 3; //unreasonable timestamp difference threshold, 1/8 duration
    spin_lock_irqsave(&st->time_stamp_lock, flags);
    timestamps_count = kfifo_len(&st->timestamps);
    while (1) {
        if (kfifo_len(&st->timestamps) >= count) {
            ...
            if (time_after64(ts, (st->last_ts + thresh))) {
                st->last_ts = ts;
                break;
            }
            //skip unreasonable timestamp
            
printk("t1=%lld, %d, %d, sys=%lld\n", st->last_ts, count, timestamps_count, get_time_ns());
        } else {
            st->last_ts += st->irq_dur_ns;
printk("t2=%lld, %d, %d, sys=%lld\n", st->last_ts, count, timestamps_count, get_time_ns());
            break;
        }
    }
    spin_unlock_irqrestore(&st->time_stamp_lock, flags);
...
//printk("t2=%lld, %d, %d, sys=%lld\n", st->last_ts, count, timestamps_count, get_time_ns());
	...
	ts = *dur;
	ts *= counter;
//printk("t1=%lld, %d, %d, %d\n", st->last_ts, count, st->irq_dur_ns, ts);
	st->last_ts += ts;
//printk("t2=%lld\n", st->last_ts);
#endif
	return 0;
...
//printk("sync=%d, t_cnt=%d, cnt=%d\n", interrupt_sync, st->timestamp_cnt, first_cnt);
        ...
        if (interrupt_sync == 1) {
    		...
    		if (ts_buf_org) {
                ts_buf_addr = (u64)ts_buf_org;
                ts_buf = (u64 *)((ts_buf_addr/sizeof(u64) + 1) * sizeof(u64));
    ...
    if (st->ts_reset == 1)
    {
        ...
        inv_prescan_data(st, dptr, target_bytes);
for (i=0; i<SENSOR_NUM_MAX; i++) {
    printk("%d, on=%d, count=%d\n", i, st->sensor[i].on, st->sensor[i].count);
}

        min_ts = 0;
        for (i=0; i<SENSOR_NUM_MAX; i++) {
            if (st->sensor[i].on && st->sensor[i].count){
                st->sensor[i].ts = st->last_ts - (u64)st->sensor[i].dur * st->sensor[i].count;
                if (min_ts == 0)
                    min_ts = st->sensor[i].ts;
                else
                {
                    if (min_ts > st->sensor[i].ts)
                        min_ts = st->sensor[i].ts;
                }                
            }
        }
        ...
        if (min_ts != 0) {
            for (i=0; i<SENSOR_NUM_MAX; i++) {
                st->sensor[i].ts = min_ts;
            }        
            st->ts_reset = 0;
printk("ts_reset, ts=%lld\n", min_ts);
        }
    }
#endif
	...
	while ((dptr - d <= target_bytes - HEADERED_NORMAL_BYTES) &&
		...
		if (sensor_ind == SENSOR_STEP) {
            ...
            printk("step pkt, hdr=0x%04x\n", hdr);
			dptr += HEADERED_NORMAL_BYTES;
			continue;
		}
		if (steps > 1) {
			inv_push_step_indicator(st, sensor_ind, steps);
            printk("step_indicator, hdr=0x%04x, steps=%d\n", hdr, steps);
        }
...
//printk("11, ts=%lld, sys=%lld, dur=%d [%d][%d, %d]\n", new_ts, get_time_ns(), st->sensor[sensor_ind].dur, sensor_ind, st->int_cnt, st->sensor[sensor_ind].sen_cnt);
        } else {
		    new_ts = st->sensor[sensor_ind].ts + (u64)st->sensor[sensor_ind].dur;
//printk("22, ts=%lld, sys=%lld, dur=%d [%d][%d, %d]\n", new_ts, get_time_ns(), st->sensor[sensor_ind].dur, sensor_ind, st->int_cnt, st->sensor[sensor_ind].sen_cnt);
        }
...
//printk("%s, cur_ts=%lld\n", __func__, get_time_ns());

#define DMP_MIN_RUN_TIME (37 * NSEC_PER_MSEC)
//	if (st->suspend_state)
//		return IRQ_HANDLED;
	if (st->wait_for_resume == 1) {
		mutex_lock(&st->suspend_resume_lock);
	}
	mutex_lock(&indio_dev->mlock);
	if (st->chip_config.dmp_on) {
...
//printk("fifo_count=%d, timestamp_cnt=%d\n", fifo_count, st->timestamp_cnt);
...
{
	return inv_check_conflict_sysfs(indio_dev);
}

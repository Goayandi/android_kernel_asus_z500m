...
extern int wcnss_irq_flag_function_rx(void);	//ASUS_BSP "for wlan wakeup trace"
...
VOID nicRxProcessDataPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
	...
	/*----------------------------------------------------------*/
	if (wcnss_irq_flag_function_rx() == 1) {

		PUINT_8 asus_data = (PUINT_8) prSwRfb->pvHeader;
		unsigned int asus_len = prSwRfb->u2PacketLen;

		int temp = 0;
		printk("[WiFi][wlan_wakeup]: nicRxProcessDataPacket: Full dump");
		for(; temp<prSwRfb->u2PacketLen; temp++)
			printk(" %.2x", asus_data[temp]);
		printk("\n");

		if(prSwRfb->u2PacketLen > 14) {
			if( (0x08==asus_data[12]) && (0x00==asus_data[13]) && (asus_len >= 38) ){
				//IPv4
				printk("[WiFi][wlan_wakeup]: (DestMac %.2x %.2x %.2x %.2x %.2x %.2x)(SrcMac %.2x %.2x %.2x %.2x %.2x %.2x)(EthType %.2x %.2x)(Protocol:%d)(SrcIP %d.%d.%d.%d)(srcPort %d)(destPort %d)\n",
				...
				printk("[WiFi][wlan_wakeup]: (EthType %.2x %.2x) (%d)\n", asus_data[12], asus_data[13], asus_len);
				...
				if( asus_len >= 38 ){
					...
					printk("[WiFi][wlan_wakeup]: (DestMac %.2x:%.2x:%.2x:%.2x:%.2x:%.2x)(SrcMac %.2x:%.2x:%.2x:%.2x:%.2x:%.2x)(EthType %.2x %.2x)(Protocol:%d)\n",
					asus_data[0], asus_data[1], asus_data[2], asus_data[3], asus_data[4], asus_data[5],
					asus_data[6], asus_data[7], asus_data[8], asus_data[9], asus_data[10], asus_data[11],
					asus_data[12], asus_data[13],
					asus_data[23] );
...
VOID nicRxProcessEventPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
	...
	if (wcnss_irq_flag_function_rx() == 1) {
		if (prEvent->ucEID != EVENT_ID_DEBUG_MSG)
			printk("[WiFi][wlan_wakeup]: nicRxProcessEventPacket: RX EVENT: ID[0x%02X] SEQ[%u] LEN[%u]\n",
			prEvent->ucEID, prEvent->ucSeqNum, prEvent->u2PacketLength);
		else
			printk("[WiFi][wlan_wakeup]: nicRxProcessEventPacket: RX EVENT: ID[0x%02X]\n", prEvent->ucEID);
	}
	//ASUS_BSP--- "for wlan wakeup trace"
...
VOID nicRxProcessMgmtPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
	...
	if (wcnss_irq_flag_function_rx() == 1){
		...
		int temp = 0;
		printk("[WiFi][wlan_wakeup]: nicRxProcessMgmtPacket: ucSubtype = %x\n", ucSubtype);
		printk("[WiFi][wlan_wakeup]: nicRxProcessMgmtPacket: Full dump");
		for(; temp<prSwRfb->u2PacketLen; temp++)
			printk(" %.2x", asus_data[temp]);
		printk("\n");
	}

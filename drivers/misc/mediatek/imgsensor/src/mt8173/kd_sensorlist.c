...
#define OV8856_OV5670_OTP_SIZE 32

static unsigned int ATD_ov5670_status;
static unsigned int ATD_ov8856_status;
#define OV5670_RESOLUTION "5M"
#define OV5670_MODULE "OV5670"
static char ov5670_OTP[200];
static char ov5670_UID[100];
#define OV8856_RESOLUTION "8M"
#define OV8856_MODULE "OV8856"
static char ov8856_OTP[200];
static char ov8856_UID[100];
		...
		u16 i2cId)
{
	...
	if (gI2CBusNum == camera_i2c_bus_num1) {
		...
		i4RetValue = i2c_transfer(g_pstI2Cclient->adapter, msg,
			ARRAY_SIZE(msg));
		if (i4RetValue != 2) {
			PK_DBG("[CAMERA SENSOR] I2C read failed!!\n");
			return -1;
...
inline static int adopt_CAMERA_HW_CheckIsAlive(void)
{
	...
	UINT32 err1 = 0;
	UINT32 i = 0;
	char ov5670_data[OV8856_OV5670_OTP_SIZE];
	char ov8856_data[OV8856_OV5670_OTP_SIZE];
	int offset = 0;
	MUINT32 sensorID = 0;
	MUINT32 retLen = 0;
				...
				if (sensorID == OV5670MIPI_SENSOR_ID) {
					...
					err =
					    ...
					    (g_invokeSocketIdx[i],
					     ...
					     (MUINT8 *) &ov5670_data, &retLen);

					offset = 0;
					for (i = 0; i < OV8856_OV5670_OTP_SIZE; i++)
						offset += scnprintf(ov5670_OTP + offset,
						sizeof(ov5670_OTP) - offset, "0x%02X ",
						ov5670_data[i]);

					offset = 0;
					for (i = 0; i < 12; i++)
						offset += scnprintf(ov5670_UID + offset,
						sizeof(ov5670_UID) - offset, "%02X",
						ov5670_data[10 + i]);

					pr_err("%s, ov5670 otp = %s\n",
					       __func__, ov5670_OTP);
					pr_err("%s, ov5670 uid = %s\n",
					       __func__, ov5670_UID);
				}
				if (sensorID == OV8856MIPI_SENSOR_ID) {
					ATD_ov8856_status = 1;
					err =
					    g_pSensorFunc->
					    SensorFeatureControl
					    (g_invokeSocketIdx[i],
					     SENSOR_FEATURE_CHECK_SENSOR_OTP,
					     (MUINT8 *) &ov8856_data, &retLen);

					offset = 0;
					for (i = 0; i < OV8856_OV5670_OTP_SIZE; i++)
						offset += scnprintf(ov8856_OTP + offset,
						sizeof(ov8856_OTP) - offset, "0x%02X ",
						ov8856_data[i]);

					offset = 0;
					for (i = 0; i < 12; i++)
						offset += scnprintf(ov8856_UID + offset,
						sizeof(ov8856_UID) - offset, "%02X",
						ov8856_data[10 + i]);

					pr_err("%s, ov8856 otp = %s\n",
					       __func__, ov8856_OTP);
					pr_err("%s, ov8856 uid = %s\n",
					       __func__, ov8856_UID);
				}
			}
		}
...
static ssize_t camera_status_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d\n", ATD_ov8856_status);
}
static ssize_t vga_status_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d\n", ATD_ov5670_status);
}

static ssize_t CameraResolution0_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%s\n", OV8856_RESOLUTION);
}
static ssize_t CameraResolution1_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%s\n", OV5670_RESOLUTION);
}

static ssize_t CameraModule0_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%s\n", OV8856_MODULE);
}
static ssize_t CameraModule1_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%s\n", OV5670_MODULE);
}

static ssize_t CameraOTP0_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%s\n", ov8856_OTP);
}
static ssize_t CameraOTP1_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%s\n", ov5670_OTP);
}

static ssize_t Camera_Unique_ID0_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%s\n", ov8856_UID);
}
static ssize_t Camera_Unique_ID1_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%s\n", ov5670_UID);
}
...
static struct device_attribute attributes[] = {
	__ATTR_RO(camera_status),
	__ATTR_RO(CameraResolution0),
	__ATTR_RO(CameraModule0),
	__ATTR_RO(CameraOTP0),
	__ATTR_RO(Camera_Unique_ID0),
	__ATTR_RO(vga_status),
	__ATTR_RO(CameraResolution1),
	__ATTR_RO(CameraModule1),
	__ATTR_RO(CameraOTP1),
	__ATTR_RO(Camera_Unique_ID1),
};
...
static int create_sysfs_interfaces(void)
{
	...
	if (IS_ERR(camera_class)) {
		...
		camera_class = NULL;
		pr_err("%s: could not allocate camera_class, ret = %d\n",
								__func__, ret);
		goto camera_class_error;
	}

	camera_dev = device_create(camera_class,
				NULL, 0, "%s", "camera");

	if (camera_dev == NULL)
		goto camera_dev_error;
	for (i = 0; i < ARRAY_SIZE(attributes); i++)
		if (device_create_file(camera_dev, attributes + i))
			goto camera_dev_create_file_error;

	return 0;

camera_dev_create_file_error:
	for ( ; i >= 0; i--)
		device_remove_file(camera_dev, attributes + i);
camera_dev_error:
...
static int CAMERA_HW_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	i4RetValue = create_sysfs_interfaces();
	if (i4RetValue)
		pr_err("camera: Failed to create sysfs class group\n");

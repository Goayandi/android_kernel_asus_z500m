...
{
	int i4RetValue = 0;
	char protection_off[2] = { (char)(0xEC), (char)(0xA3) };
	char DLC_MCLK[2] = { (char)(0xA1), (char)(0x0D) };
	char T_SRC[2] = { (char)(0xF2), (char)(0x08) };
	char protection_on[2] = { (char)(0xDC), (char)(0x51) };
	...
	i4RetValue = i2c_master_send(g_pstAF_I2Cclient, protection_off, 2);
	i4RetValue = i2c_master_send(g_pstAF_I2Cclient, DLC_MCLK, 2);
	i4RetValue = i2c_master_send(g_pstAF_I2Cclient, T_SRC, 2);
	i4RetValue = i2c_master_send(g_pstAF_I2Cclient, protection_on, 2);
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if ((a_u4Position > g_u4AF_MACRO) || (a_u4Position < g_u4AF_INF)) {
		LOG_INF("out of range {macro, inf}={%lu, %lu}\n", g_u4AF_MACRO, g_u4AF_INF);
		return -EINVAL;
	}
...
int S2034AF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	...
	if (*g_pAF_Opened == 2) {
		...
		msleep(20);
                */
		//reduce the noise of VCM when releasing camera
		while(step >=0) {
			if (step < 0)
				step = 0;
			s4AF_WriteReg(step);

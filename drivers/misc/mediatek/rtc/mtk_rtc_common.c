...
{
	unsigned long time, flags;
	unsigned long UTS_ori_time;
	struct rtc_time UTS_ori_tm, UTS_new_tm;
	...
	spin_lock_irqsave(&rtc_lock, flags);
	hal_rtc_get_tick_time(&UTS_ori_tm);
	spin_unlock_irqrestore(&rtc_lock, flags);
	...
	UTS_ori_tm.tm_year += RTC_MIN_YEAR_OFFSET;
	UTS_ori_tm.tm_mon--;
	rtc_tm_to_time(&UTS_ori_tm, &UTS_ori_time);
	UTS_ori_time -= sys_tz.tz_minuteswest * 60;
	rtc_time_to_tm(UTS_ori_time, &UTS_ori_tm);

	time -= sys_tz.tz_minuteswest * 60;
	rtc_time_to_tm(time, &UTS_new_tm);

	ASUSEvtlog("[UTS] RTC update: Current Datetime: %04d-%02d-%02d %02d:%02d:%02d,"
					"Update Datetime: %04d-%02d-%02d %02d:%02d:%02d\n",
				UTS_ori_tm.tm_year + 1900, UTS_ori_tm.tm_mon + 1, UTS_ori_tm.tm_mday,
				UTS_ori_tm.tm_hour, UTS_ori_tm.tm_min, UTS_ori_tm.tm_sec,
				UTS_new_tm.tm_year + 1900, UTS_new_tm.tm_mon + 1, UTS_new_tm.tm_mday,
				UTS_new_tm.tm_hour, UTS_new_tm.tm_min, UTS_new_tm.tm_sec);

...
#include "../../misc/mediatek/base/power/mt8173/mt_spm_sleep.h"
			...
			bool input)
{
	...
	pr_debug("fwq gpioapi set_direction pin=%d,dir_input=%d\n", offset, input);
...
static void mtk_gpio_set(struct gpio_chip *chip, unsigned offset, int value)
{
	...
	pr_debug("fwq pinctrl set gpio out pin=%d,value=%d\n", offset, value);
...
{
	pr_debug("fwq pinctrl set mtk_pconf_set_direction pin=%d,value=%d,para=%d\n", pin, value, param);
	if (pctl->devdata->mt_set_gpio_dir)
		...
		unsigned int pin, bool enable, bool isup, unsigned int arg)
{
	...
	pr_debug("fwq pinctrl set pullen & pull_select en=%d pin=%d,isup=%d\n", enable, pin, isup);
		...
		unsigned long pin, unsigned long mode)
{
	...
	pr_debug("fwq pinctrl set mode pin=%ld,mode=%ld\n", pin, mode);
...
{
	pr_debug("fwq pinctrl set out pin=%d,out=%d\n", offset, value);
...
static int mtk_gpio_get_direction(struct gpio_chip *chip, unsigned offset)
{
	...
	pr_debug("fwq pinctrl get_direction pin=%d\n", offset);
...
static int mtk_eint_syscore_suspend(void)
{
	return 0;
}

static struct mt_wake_event eint_event = {
	.domain = "EINT",
};

static struct mtk_pinctrl *pctl_inst;

static void mtk_eint_syscore_resume(void)
{
	unsigned int status, eint_num;
	int offset, index;

	struct mtk_pinctrl *pctl = pctl_inst;
	const struct mtk_eint_offsets *eint_offsets = &pctl->devdata->eint_offsets;
	...
	if (we && we->domain && !strcmp(we->domain, "SPM") && we->code == 6) {
		for (eint_num = 0; eint_num < pctl->devdata->ap_num; eint_num += 32) {
			...
			if (status) {
				...
				spm_report_wakeup_event(&eint_event, index);
				break;
			}
		}
	}
...
static struct syscore_ops mt_eint_syscore_ops = {
	.suspend = mtk_eint_syscore_suspend,
	.resume = mtk_eint_syscore_resume,
};
		...
		struct regmap *regmap)
{
	...
	if (pctl->devdata->ap_num != 0 && !pctl_inst) {
		pctl_inst = pctl;

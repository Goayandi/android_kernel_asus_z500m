...
int battery_count_99_up = 0;
int battery_last_SOC = 0;
int BATTERY_99_UP_DURATION = 60;
...
static int acin_suspend = 0; //default ac-in cannot suspend
...
static struct mt_battery_charging_custom_data default_charging_data = {
	...
	.sync_to_real_tracking_time = 10,	/* 10 second */
...
static DEFINE_MUTEX(TypeC_charger_type_mutex);
...
#if defined(CONFIG_ANALOGIX_OHIO) || defined(CONFIG_TCPC_RT1711)
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		if (asus_show_qc_flag) {
			if (BMT_status.SOC < 70)
...
static ssize_t show_ACin_Suspend(struct device *dev, struct device_attribute *attr, char *buf)
{
	return snprintf(buf, sizeof(char) * 2 + 1, "%d\n", acin_suspend);
}

static ssize_t store_ACin_Suspend(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	int ret, cur;
...
static DEVICE_ATTR(ACin_Suspend, 0664, show_ACin_Suspend, store_ACin_Suspend);

static void mt_battery_update_EM(struct battery_data *bat_data)
{
	if (bat_data->BAT_CAPACITY < BMT_status.UI_SOC)
		battery_last_SOC = bat_data->BAT_CAPACITY;
	bat_data->BAT_CAPACITY = BMT_status.UI_SOC;
	if ((bat_data->BAT_CAPACITY == 99) && (battery_last_SOC == 98) && BMT_status.charger_exist)
		battery_count_99_up++;
	if ((bat_data->BAT_CAPACITY >= 99) && (battery_last_SOC == 98) && BMT_status.charger_exist && (battery_count_99_up > BATTERY_99_UP_DURATION))  /* 600sec treat 99 as 100 percent for better user experience */
		bat_data->BAT_CAPACITY = 100;
	if ((!BMT_status.charger_exist) || (BMT_status.UI_SOC != 99))
		battery_count_99_up = 0;
	...
	if ((bat_data->BAT_CAPACITY == 100) && BMT_status.charger_exist)
...
static bool mt_battery_100Percent_tracking_check(void)
{
	...
	if (smb1351_skip_battery_100Percent_tracking()) {
		battery_log(BAT_LOG_FULL,
				"Skip 100percent tracking due to not 4.34V full-charging.\n");
		return false;
	}
#endif
...
{
	if ((BMT_status.charger_type == TYPEC_PD_9V_CHARGER &&
		(new_type > 0 && new_type != TYPEC_PD_9V_CHARGER)) ||
		(BMT_status.charger_type == TYPEC_PD_5V_CHARGER &&
		(new_type > 0 && new_type != TYPEC_PD_5V_CHARGER))) {
		if (new_type != TYPEC_PD_NO_READY_CHARGER) {
			pr_warn("%s: return because the PD charging", __func__);
			return;
		}
	}
...
void bat_update_TypeC_charger_type(int new_type)
{
//	if ((BMT_status.charger_type == TYPEC_PD_9V_CHARGER &&
//				(new_type > 0 && new_type != TYPEC_PD_9V_CHARGER)) ||
//			(BMT_status.charger_type == TYPEC_PD_5V_CHARGER &&
...
//	}
	...
	pr_notice("===> Entering bat_update_TypeC_charger_type ==>\n");

	mutex_lock(&TypeC_charger_type_mutex);
	BMT_status.TypeC_charger_type = new_type;
	mutex_unlock(&TypeC_charger_type_mutex);
	pr_notice("BMT_status.TypeC_charger_type is ==> %d\n", BMT_status.TypeC_charger_type);
	battery_log(BAT_LOG_CRTI, "update new TypeC charger type: %d\n", new_type);
	wake_up_bat();
}


bool free_COS_wakelock(void)
{
	if (!BMT_status.charger_exist) return 0;
		pr_notice("BMT_status.bat_full...%d\n",BMT_status.bat_full);
		pr_notice("get_boot_mode ...%d\n",get_boot_mode());
		pr_notice("!smb1351_skip_battery_100Percent_tracking ...%d\n",!smb1351_skip_battery_100Percent_tracking());
	if (BMT_status.bat_full && (get_boot_mode() == KERNEL_POWER_OFF_CHARGING_BOOT) && !smb1351_skip_battery_100Percent_tracking()){
		pr_notice(".....3 conditions all hold...........\n");
		return 1;
	}
	return 0;
}
...
static void mt_battery_charger_detect_check(void)
{
	...
	if (upmu_is_chr_det() == true) {
		...
		if (!BMT_status.charger_exist)
			acin_suspend ? wake_lock_timeout(&battery_suspend_lock, HZ / 2) : wake_lock(&battery_suspend_lock);
...
static int battery_probe(struct platform_device *pdev)
{
	...
	/* For EM */
	{
		...
		ret_device_file = device_create_file(dev, &dev_attr_ACin_Suspend);
#if defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_SUPPORT)

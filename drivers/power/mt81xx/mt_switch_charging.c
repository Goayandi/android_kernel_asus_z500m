	...
	if (g_ftm_battery_flag) {
		...
		if (BMT_status.charger_type == STANDARD_HOST) {
			...
			if (mt_usb_pd_get_current())
				...
				g_temp_input_CC_value = 150000;
			g_temp_CC_value = p_bat_charging_data->ac_charger_current;
		} else if (BMT_status.charger_type == TYPEC_PD_9V_CHARGER) {
			if (mt_usb_pd_get_current())
				g_temp_input_CC_value = mt_usb_pd_get_current() * 100;
			else
				g_temp_input_CC_value = 150000;
...
static bool charging_full_check(void)
{
	...
	if (status) {
		battery_log(BAT_LOG_CRTI, "[BATTERY] full_check_count %d !\n", full_check_count);
		full_check_count++;
...
#else
	pr_info("222 BMT_status.bat_charging_state : %d\n", BMT_status.bat_charging_state);
	if (BMT_status.charger_exist) {
		if (BMT_status.bat_charging_state == CHR_BATFULL) {
			BAT_BatteryFullAction();
			pr_info("battery full, still update charger type\n");
			smb1351_charging_algorithm(No_Wait);

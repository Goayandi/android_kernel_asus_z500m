...
unsigned int mfg_power_on_start = 0, mfg_power_on_end = 0;
unsigned int mfg_power_off_start = 0, mfg_power_off_end = 0;
...
static int scpsys_power_on(struct generic_pm_domain *genpd)
{
	...
	if (scpd->clk) {
		...
		if (ret)
		{
			dev_err(scp->dev, "Failed to clk enable scpsys_power_on %s\n", scpd->data->name);
			goto out;
		}
	}
	...
	while (!(readl(scp->base + SPM_PWR_STATUS) & data->sta_mask) ||
		...
		if (time_after(jiffies, expired)) {
			ret = -EIO;
			dev_err(scp->dev, "Failed to PWR_ACK scpsys_power_on %s\n", scpd->data->name);
			goto out;
		}
	}
	...
	while (sram_pdn_ack && (readl(ctl_addr) & sram_pdn_ack)) {
		...
		if (time_after(jiffies, expired)) {
			ret = -EIO;
			dev_err(scp->dev, "Failed to SRAM_PDN_ACK scpsys_power_on %s\n", scpd->data->name);
			goto out;
		}
	}
	...
	if (data->bus_prot_mask) {
		...
		if (ret)
		{
			dev_err(scp->dev, "Failed to CLR_BUS_PROT scpsys_power_on %s\n", scpd->data->name);
			goto out;
		}
	}
...
static int scpsys_power_off(struct generic_pm_domain *genpd)
{
	...
	if (data->bus_prot_mask) {
		...
		if (ret)
		{
			dev_err(scp->dev, "Failed to SET_BUS_PROT scpsys_power_off %s\n", scpd->data->name);
			goto out;
		}
	}
	...
	while ((readl(ctl_addr) & sram_pdn_ack) != sram_pdn_ack) {
		...
		if (time_after(jiffies, expired)) {
			ret = -EIO;
			dev_err(scp->dev, "Failed to SRAM_PDN_ACK scpsys_power_off %s\n", scpd->data->name);
			goto out;
		}
	}
	...
	while ((readl(scp->base + SPM_PWR_STATUS) & data->sta_mask) ||
		...
		if (time_after(jiffies, expired)) {
			ret = -EIO;
			dev_err(scp->dev, "Failed to PWR_ACK scpsys_power_off %s\n", scpd->data->name);
			goto out;
		}
	}

...
static struct device_attribute tcpc_device_attributes[] = {
	...
	TCPC_DEVICE_ATTR(typec_status, S_IRUGO),
	TCPC_DEVICE_ATTR(cc_status, S_IRUGO),
};
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	int i = 0;
	int vmin, vmax, ioper, result, cc1, cc2;
	...
	case TCPC_DESC_TYPEC_STATUS:
		...
		snprintf(buf, 256, "%d\n", result);
		break;
	case TCPC_DESC_CC_STATUS:
		if (tcpci_get_cc(tcpc, &cc1, &cc2) < 0)
		{
			pr_info("%s get CC status fail\n", __func__);
			result = -1;
		} else {
			pr_info("%s CC status: %d/%d\r\n", __func__, cc1, cc2);
			if (cc1 == cc2)
				result = 0;   /* cable unplug */
			else if (cc1 > 0)
				result = 1;
			else if (cc2 > 0)
				result = 2;

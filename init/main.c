...
enum DEVICE_PROJID g_ASUS_projID = PROJECT_INVALID;
enum DEVICE_LCMID g_ASUS_lcmID = LCM_INVALID;
...
static int of_get_pcbid_data(void)
{
	...
	pcbid_str = of_get_property(np, "lcm_id", NULL);
	if (pcbid_str == NULL)
		pr_err("Fail to get LCM ID though dt\n");
	else if (!kstrtoint(pcbid_str, 0, &value)) {
		g_ASUS_lcmID = value;
		pr_info("PCBID: Get LCM ID: %d\n", g_ASUS_lcmID);
	}

...
struct snd_soc_codec *rt5659_codec;
EXPORT_SYMBOL(rt5659_codec);
static bool codec_ctl_status;
...
/**
 * rt5659_index_write - Write private register.
 ...
 */
static int rt5659_index_write(struct snd_soc_codec *codec,
		unsigned int reg, unsigned int value)
{
	int ret;
	ret = snd_soc_write(codec, RT5659_PRIV_INDEX, reg);
	if (ret < 0) {
		dev_err(codec->dev, "Failed to set private addr: %d\n", ret);
		pr_err("%s(%d) rt5659.c fail\n",__func__,__LINE__);
		goto err;
	}
	ret = snd_soc_write(codec, RT5659_PRIV_DATA, value);
	pr_err("%s(%d) rt5659.c complete write \n",__func__,__LINE__);
	if (ret < 0) {
...
/**
 * rt5659_index_read - Read private register.
 * @codec: SoC audio codec device.
 * @reg: Private register index.
 *
 ...
 */
static unsigned int rt5659_index_read(
	struct snd_soc_codec *codec, unsigned int reg)
{
	...
	return snd_soc_read(codec, RT5659_PRIV_DATA);
}

/**
 * rt5659_index_update_bits - update private register bits
 * @codec: audio codec
 * @reg: Private register index.
 * @mask: register mask
 * @value: new value
 ...
 */
static int rt5659_index_update_bits(struct snd_soc_codec *codec,
	unsigned int reg, unsigned int mask, unsigned int value)
{
	unsigned int old, new;
	int change, ret;

	ret = rt5659_index_read(codec, reg);
	if (ret < 0) {
	...
	change = old != new;
	if (change) {
		ret = rt5659_index_write(codec, reg, new);
		if (ret < 0) {
			dev_err(codec->dev,
 ...
 */
static void rt5659_enable_micbias1(struct snd_soc_codec *codec, int enabled);
...
int rt5659_headset_detect(struct snd_soc_codec *codec, int jack_insert)
{
	...
	if (jack_insert) {
		snd_soc_dapm_force_enable_pin(dapm,
			"Mic Det Power");
		snd_soc_dapm_sync(dapm);
		rt5659_enable_micbias1(codec, 1);
		...
		/* disable LDO2/Mic Det Power/MicBias1 if jack remove. */
		rt5659_enable_micbias1(codec, 0);
		snd_soc_dapm_sync(dapm);
...
static void rt5659_enable_micbias1(struct snd_soc_codec *codec, int enabled)
{
	...
	if (enabled) {
		pr_info("%s: enable headset micbias\n", __func__);
		snd_soc_dapm_force_enable_pin(dapm,
			"Mic Det Power");
		snd_soc_dapm_force_enable_pin(dapm, "MICBIAS1");
		snd_soc_dapm_sync(dapm);
		...
		/* power on internal LDO2 for MIC_VDD */
		snd_soc_update_bits(codec, RT5659_PWR_ANLG_3,
			RT5659_PWR_LDO2, RT5659_PWR_LDO2);
		/* select 1.8V micbias */
		snd_soc_update_bits(codec, RT5659_MICBIAS_1,
			0xC000, 0xC000);
		/* power on micbias1 */
		snd_soc_update_bits(codec, RT5659_PWR_ANLG_2,
			RT5659_PWR_MB1, RT5659_PWR_MB1);
	} else {
		pr_info("%s: disable headset micbias\n", __func__);
		/* power down internal LDO2 for MIC_VDD */
		snd_soc_update_bits(codec, RT5659_PWR_ANLG_3,
			RT5659_PWR_LDO2, 0);
		snd_soc_dapm_disable_pin(dapm, "MICBIAS1");
		snd_soc_dapm_disable_pin(dapm, "Mic Det Power");
...
static void rt5659_jack_detect_work(struct work_struct *work)
{
	...
	/* jack in */
	if ((rt5659->jack_type & SND_JACK_HEADSET) == 0) {
		/* jack was out, report jack type */
		// report = rt5659_headset_detect(rt5659->codec, 1);
		return ; // we only take care of hook
	} else {
...
static const struct snd_kcontrol_new rt5659_snd_controls[] = {
	/* DRC settings */
	/* mx-0300h DRC Control 1 */
	SOC_SINGLE("DRC Noise Gate Hold Time", RT5659_DRC1_CTRL_1, 12, 7, 0),
	SOC_SINGLE("DRC Digital Pre-BOOST", RT5659_DRC1_CTRL_1, 6, 39, 0),
	SOC_SINGLE("DRC Right Channel Digital Post-BOOST", RT5659_DRC1_CTRL_1, 0, 63, 0),

	/* mx-0301h DRC Control 2 */
	SOC_SINGLE("DRC Enable", RT5659_DRC1_CTRL_2, 14, 3, 0),
	SOC_SINGLE("Select DRC attack rate", RT5659_DRC1_CTRL_2, 8, 18, 0),
	SOC_SINGLE("DRC Rate Control", RT5659_DRC1_CTRL_2, 5, 7, 0),
	SOC_SINGLE("Select DRC recovery rate", RT5659_DRC1_CTRL_2, 0, 18, 0),

	/* mx-0302h DRC Control 3 */
	SOC_SINGLE("DRC Compression-2 Ratio Selection", RT5659_DRC1_CTRL_3, 14, 3, 0),
	SOC_SINGLE("DRC Left Channel Digital Post-BOOST", RT5659_DRC1_CTRL_3, 8, 63, 0),
	SOC_SINGLE("DRC Function Control", RT5659_DRC1_CTRL_3, 7, 1, 0),
	SOC_SINGLE("DRC Compression-1 Ratio Selection", RT5659_DRC1_CTRL_3, 5, 3, 0),
	SOC_SINGLE("DRC Noise Gate Drop Mode Control", RT5659_DRC1_CTRL_3, 4, 1, 0),
	SOC_SINGLE("ALC DRC Expansion Ratio Control", RT5659_DRC1_CTRL_3, 0, 3, 0),

	/* mx-0303h DRC Control 4 */
	SOC_SINGLE("Select Boost Compensation gain", RT5659_DRC1_CTRL_4, 12, 15, 0),
	SOC_SINGLE("DRC Threshold Level Control", RT5659_DRC1_CTRL_4, 5, 46, 0),
	SOC_SINGLE("Noise Gate Threshold", RT5659_DRC1_CTRL_4, 0, 31, 0),

	/* mx-0304h DRC Control 5 */
	SOC_SINGLE("DRC thmin Mode Fast Recover Control", RT5659_DRC1_CTRL_5, 14, 1, 0),
	SOC_SINGLE("Enable Noise Gate function", RT5659_DRC1_CTRL_5, 13, 1, 0),
	SOC_SINGLE("DRC Hold Gain when Noise Gate Mode", RT5659_DRC1_CTRL_5, 12, 1, 0),
	SOC_SINGLE("DRC Limiter Threshold 2 Control", RT5659_DRC1_CTRL_5, 6, 63, 0),
	SOC_SINGLE("DRC Limiter Threshold Control", RT5659_DRC1_CTRL_5, 0, 63, 0),

	/* mx-0308h DRC Bass Control 1 */
	SOC_SINGLE("DRC Bass Noise Gate Hold Time", RT5659_DRC2_CTRL_1, 12, 7, 0),
	SOC_SINGLE("DRC Bass Digital Pre-BOOST", RT5659_DRC2_CTRL_1, 6, 39, 0),
	SOC_SINGLE("DRC Bass Right Channel Digital Post-BOOST", RT5659_DRC2_CTRL_1, 0, 63, 0),

	/* mx-0309h DRC Bass Control 2 */
	SOC_SINGLE("DRC Bass Enable", RT5659_DRC2_CTRL_2, 15, 1, 0),
	SOC_SINGLE("Select DRC bass attack rate", RT5659_DRC2_CTRL_2, 8, 18, 0),
	SOC_SINGLE("DRC Bass Rate Control", RT5659_DRC2_CTRL_2, 5, 7, 0),
	SOC_SINGLE("Select DRC Bass recovery rate", RT5659_DRC2_CTRL_2, 0, 18, 0),

	/* mx-030ah DRC Bass Control 3 */
	SOC_SINGLE("DRC Bass Compression-2 Ratio Selection", RT5659_DRC2_CTRL_3, 14, 3, 0),
	SOC_SINGLE("DRC Bass Left Channel Digital Post-BOOST", RT5659_DRC2_CTRL_3, 8, 63, 0),
	SOC_SINGLE("DRC Bass Function Control", RT5659_DRC2_CTRL_3, 7, 1, 0),
	SOC_SINGLE("DRC Bass Compression-1 Ratio Selection", RT5659_DRC2_CTRL_3, 5, 3, 0),
	SOC_SINGLE("DRC Bass Noise Gate Drop Mode Control", RT5659_DRC2_CTRL_3, 4, 1, 0),
	SOC_SINGLE("DRC Bass DRC Expansion Ratio Control", RT5659_DRC2_CTRL_3, 0, 3, 0),

	/* mx-030bh DRC Bass Control 4 */
	SOC_SINGLE("Select Bass Boost Compensation gain", RT5659_DRC2_CTRL_4, 12, 15, 0),
	SOC_SINGLE("DRC Bass Threshold Level Control", RT5659_DRC2_CTRL_4, 5, 46, 0),
	SOC_SINGLE("Bass Noise Gate Threshold", RT5659_DRC2_CTRL_4, 0, 31, 0),

	/* mx-030ch DRC Bass Control 5 */
	SOC_SINGLE("DRC Bass thmin Mode Fast Recover Control", RT5659_DRC2_CTRL_5, 14, 1, 0),
	SOC_SINGLE("Enable Bass Noise Gate function", RT5659_DRC2_CTRL_5, 13, 1, 0),
	SOC_SINGLE("DRC Bass Hold Gain when Noise Gate Mode", RT5659_DRC2_CTRL_5, 12, 1, 0),
	SOC_SINGLE("DRC Bass Limiter Threshold 2 Control", RT5659_DRC2_CTRL_5, 6, 63, 0),
	SOC_SINGLE("DRC Bass Limiter Threshold Control", RT5659_DRC2_CTRL_5, 0, 63, 0),
...
static int rt5659_sto1_adc_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
/*
	struct snd_soc_codec *codec = snd_soc_dapm_to_codec(w->dapm);
	...
	case SND_SOC_DAPM_PRE_PMU:
		snd_soc_update_bits(codec, RT5659_ASRC_2,
			RT5659_AD_STO1_T_MASK, 0x0);
		break;
	case SND_SOC_DAPM_POST_PMU:
		mdelay(5);
		snd_soc_update_bits(codec, RT5659_ASRC_2,
			RT5659_AD_STO1_T_MASK, 0x1);
		break;
	default:
		return 0;
	}
...
static int rt5659_adc_rec1_lr_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = snd_soc_dapm_to_codec(w->dapm);
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		rt5659_update_eqmode(codec, EQ_CH_ADC_STEREO_L, Z500M_HEADSET_MIC_EQ_L);
		rt5659_update_eqmode(codec, EQ_CH_ADC_STEREO_R, Z500M_HEADSET_MIC_EQ_R);
		break;

	case SND_SOC_DAPM_POST_PMD:
		rt5659_update_eqmode(codec, EQ_CH_ADC_STEREO_L, NORMAL);
		rt5659_update_eqmode(codec, EQ_CH_ADC_STEREO_R, NORMAL);
		break;
...
static int rt5659_adc_rec2_lr_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = snd_soc_dapm_to_codec(w->dapm);
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		rt5659_update_eqmode(codec, EQ_CH_ADC_STEREO_L, Z500M_BOARD_MIC_EQ_L);
		rt5659_update_eqmode(codec, EQ_CH_ADC_STEREO_R, Z500M_BOARD_MIC_EQ_R);
		break;
...
static const struct snd_soc_dapm_widget rt5659_dapm_widgets[] = {
	...
	/* ASRC */
	SND_SOC_DAPM_SUPPLY_S("I2S1 ASRC", 2, RT5659_ASRC_1,
		RT5659_I2S1_ASRC_SFT, 0, NULL, 0),
	SND_SOC_DAPM_SUPPLY_S("I2S2 ASRC", 2, RT5659_ASRC_1,
		RT5659_I2S2_ASRC_SFT, 0, NULL, 0),
	SND_SOC_DAPM_SUPPLY_S("I2S3 ASRC", 2, RT5659_ASRC_1,
		RT5659_I2S3_ASRC_SFT, 0, NULL, 0),
	SND_SOC_DAPM_SUPPLY_S("DAC STO ASRC", 2, RT5659_ASRC_1,
		RT5659_DAC_STO_ASRC_SFT, 0, NULL, 0),
	SND_SOC_DAPM_SUPPLY_S("DAC Mono L ASRC", 2, RT5659_ASRC_1,
		RT5659_DAC_MONO_L_ASRC_SFT, 0, NULL, 0),
	SND_SOC_DAPM_SUPPLY_S("DAC Mono R ASRC", 2, RT5659_ASRC_1,
		RT5659_DAC_MONO_R_ASRC_SFT, 0, NULL, 0),
	SND_SOC_DAPM_SUPPLY_S("ADC STO1 ASRC", 2, RT5659_ASRC_1,
		RT5659_ADC_STO1_ASRC_SFT, 0, NULL, 0),
	SND_SOC_DAPM_SUPPLY_S("ADC Mono L ASRC", 2, RT5659_ASRC_1,
		RT5659_ADC_MONO_L_ASRC_SFT, 0, NULL, 0),
	SND_SOC_DAPM_SUPPLY_S("ADC Mono R ASRC", 2, RT5659_ASRC_1,
	...
	*/
	SND_SOC_DAPM_PGA_S("BST1", 1, RT5659_PWR_ANLG_2, RT5659_PWR_BST1_P_BIT,
		0, rt5659_adc_rec1_lr_event, SND_SOC_DAPM_POST_PMU |
		SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_PGA_S("BST2", 1, RT5659_PWR_ANLG_2, RT5659_PWR_BST2_P_BIT,
		0, rt5659_adc_rec2_lr_event, SND_SOC_DAPM_POST_PMU |
		SND_SOC_DAPM_POST_PMD),
	...
	SND_SOC_DAPM_SUPPLY_S("ADC1 L Power", 1, RT5659_PWR_DIG_1,
		RT5659_PWR_ADC_L1_BIT, 0, NULL, 0),
	SND_SOC_DAPM_SUPPLY_S("ADC1 R Power", 1, RT5659_PWR_DIG_1,
		RT5659_PWR_ADC_R1_BIT, 0, NULL, 0),
	SND_SOC_DAPM_SUPPLY_S("ADC2 L Power", 1, RT5659_PWR_DIG_2,
		RT5659_PWR_ADC_L2_BIT, 0, NULL, 0),
	SND_SOC_DAPM_SUPPLY_S("ADC2 R Power", 1, RT5659_PWR_DIG_2,
		RT5659_PWR_ADC_R2_BIT, 0, NULL, 0),
	SND_SOC_DAPM_SUPPLY_S("ADC1 clock", 1, SND_SOC_NOPM, 0, 0, set_adc_clk,
		SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMD),
	SND_SOC_DAPM_SUPPLY_S("ADC2 clock", 1, SND_SOC_NOPM, 0, 0, set_adc_clk,
		SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMD),
	...
	SND_SOC_DAPM_SUPPLY_S("ADC Stereo1 Filter", 1, RT5659_PWR_DIG_2,
		RT5659_PWR_ADC_S1F_BIT, 0, rt5659_sto1_adc_event,
...
#else
	SND_SOC_DAPM_PGA("Stereo1 ADC Volume L", SND_SOC_NOPM,
		0, 0, NULL, 0),
	SND_SOC_DAPM_PGA("Stereo1 ADC Volume R", SND_SOC_NOPM,
		0, 0, NULL, 0),
#endif
	/* Digital Interface */
	...
	/* DAC Mixer */
	SND_SOC_DAPM_SUPPLY_S("DAC Stereo1 Filter", 1, RT5659_PWR_DIG_2,
		RT5659_PWR_DAC_S1F_BIT, 0, NULL, 0),
	SND_SOC_DAPM_SUPPLY_S("DAC Mono Left Filter", 1, RT5659_PWR_DIG_2,
		RT5659_PWR_DAC_MF_L_BIT, 0, NULL, 0),
	SND_SOC_DAPM_SUPPLY_S("DAC Mono Right Filter", 1, RT5659_PWR_DIG_2,
	...
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	if (dai->id == RT5659_AIF2)
		rt5659->lrck[dai->id] = 192000;
...
static int audio_codec_status_show(struct seq_file *s, void *unused)
{
	seq_printf(s, "%d\n", codec_ctl_status);
	return 0;
}

static int audio_codec_status_open(struct inode *inode, struct file *file)
{
	return single_open(file, audio_codec_status_show, NULL);
}

const struct file_operations audio_codec_status_fops = {
	.open = audio_codec_status_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

static struct dentry *audio_codec_dbg_root;

static void check_codec_ctl_status(void)
{
	unsigned int reg;

	if (rt5659_codec != NULL) {
		reg = snd_soc_read(rt5659_codec, RT5659_DEVICE_ID);
		if (reg == DEVICE_ID)
			codec_ctl_status = true;
		else
			codec_ctl_status = false;
	}
}


uint32_t last_register = 0;
static ssize_t rt5659_register_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	int val;

	val = snd_soc_read(rt5659_codec, last_register);
	return sprintf(buf, "%04x %04x\n", last_register, val);
}

/* Any write to the keyclick attribute will trigger the keyclick event */
static ssize_t rt5659_register_store(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t count)
{
	unsigned long reg, val, num;
	char * last;
	num = sscanf(buf, "%x %x", &reg, &val);
	if (num < 1)
		return count;
	last_register = reg;
	if (num < 2)
		return count;
	printk("stored %04x %04x\n", last_register, val);
	snd_soc_write(rt5659_codec, last_register, val);

	return count;
}

static DEVICE_ATTR(codec_register, 0644, rt5659_register_show, rt5659_register_store);


static int rt5659_probe(struct snd_soc_codec *codec)
{
	struct rt5659_priv *rt5659 = snd_soc_codec_get_drvdata(codec);
	struct dentry *codec_dentry;
	int ret;
	pr_info("%s +\n", __func__);
	...
	rt5659_codec = codec;
	/* 48k*256 */
	rt5659->sysclk = 49152000 * 2;/*12288000;*/
	/* avoid DAC distortion since mclk is too slow */
//	snd_soc_write(codec, RT5659_ADDA_CLK_1, 0x0000);
	/* choose gpio function as I2S for I2S3 port */
	snd_soc_update_bits(codec, RT5659_GPIO_CTRL_2, 0x8000, 0x8000);
	/* enable to control power by register */
	snd_soc_update_bits(codec, RT5659_EJD_CTRL_1, 0x0040, 0x0040);
	snd_soc_update_bits(codec, RT5659_IL_CMD_3, 0xf000, 0xc000);

	/* for PLL */
	//snd_soc_write(codec, RT5659_PLL_CTRL_1, 0x0000);
	//snd_soc_write(codec, RT5659_PLL_CTRL_2, 0x0800);
	snd_soc_write(codec, RT5659_DUMMY_4, 0x0001);
	//snd_soc_write(codec, RT5659_GLB_CLK, 0x4000);
	snd_soc_write(codec, RT5659_ADDA_CLK_1, 0x03ba);
	/* for ASRC */
	snd_soc_write(codec, RT5659_ASRC_2, 0x0005);
	snd_soc_write(codec, RT5659_ASRC_3, 0x0066);
	snd_soc_write(codec, RT5659_ASRC_8, 0x0200);
	snd_soc_write(codec, RT5659_ASRC_10, 0x0007);


	audio_codec_dbg_root = debugfs_create_dir("audio_codec_debug", NULL);
	codec_dentry = debugfs_create_file("audio_codec_status", S_IRUGO,
		audio_codec_dbg_root, codec->dev, &audio_codec_status_fops);
	codec_ctl_status = false;
	check_codec_ctl_status();

	mt8173_accdet_enable(codec);
	snd_soc_update_bits(codec, RT5659_GPIO_CTRL_1,
                        RT5659_GP1_PIN_MASK, RT5659_GP1_PIN_IRQ);

#ifdef RTK_IOCTL
#if defined(CONFIG_SND_HWDEP) || defined(CONFIG_SND_HWDEP_MODULE)
	ioctl_ops->index_write = rt5659_index_write;
	ioctl_ops->index_read = rt5659_index_read;
	ioctl_ops->index_update_bits = rt5659_index_update_bits;
	ioctl_ops->ioctl_common = rt5659_ioctl_common;
		    ...
		    const struct i2c_device_id *id)
{
	...
	ret = device_create_file(&i2c->dev, &dev_attr_codec_register);
	pr_info("%s -\n", __func__);
	return 0;
}
